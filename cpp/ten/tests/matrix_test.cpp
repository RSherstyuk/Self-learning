...
TYPED_TEST(MatrixTest, AlignmentAfterConstruction) {
    ...
    ASSERT_EQ(result_data.size(), 4) << "Количество строк должно соответствовать исходному.";

    size_t expected_size = 4;
    for (const auto& row : result_data) {
        EXPECT_EQ(row.size(), expected_size) << "Все строки должны быть выравнены до максимальной длины.";
    }
...
TYPED_TEST(MatrixTest, AlreadyRectangular) {
    ...
    for (const auto& row : result_data) {
        EXPECT_EQ(row.size(), 2) << "Размер должен остаться 2, так как выравнивание не требуется.";
    }
...
TYPED_TEST(MatrixTest, WithEmptyRows) {
    ...
    ASSERT_EQ(result_data.size(), 3);
    for (const auto& row : result_data) {
        EXPECT_EQ(row.size(), expected_size) << "Все строки должны быть выравнены до 3.";
    }

    EXPECT_EQ(result_data[0][0], (TypeParam)0) << "Пустая строка должна заполниться значениями по умолчанию.";
    EXPECT_EQ(result_data[2][1], (TypeParam)0) << "Короткая строка должна заполниться значениями по умолчанию.";
...
TEST(MatrixIntTest, EmptyMatrix) {
    ...
    EXPECT_TRUE(result_data.empty()) << "Конструктор пустой матрицы должен давать пустые данные.";
}
...
TEST(MatrixDoubleTest, AllEmptyRows) {
    ...
    for (const auto& row : result_data) {
        EXPECT_EQ(row.size(), 0) << "Все строки должны остаться длиной 0, так как максимальная длина 0.";
    }
}
